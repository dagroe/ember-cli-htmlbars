...
const { SourceMap } = require('magic-string');
...
module.exports = class ColocatedTemplateProcessor extends Plugin {
  ...
  generateSourceMappings(content) {
    let newFileMappings = [[], []];
    ...
    for (let lineIndex = 0; lineIndex < arr.length; lineIndex++) {
      let offset = lastLineLength ? -lastLineLength : 0;
      // first entry is relative next line but column offset by previous line length
      let lineMapping = [[0, 0, lineIndex === 0 ? 0 : 1, offset]];
      let line = arr[lineIndex];
      for (let columnIndex = 0; columnIndex < line.length; columnIndex++) {
        lineMapping.push([1, 0, 0, 1]);
      }
      lastLineLength = line.length;
      newFileMappings.push(lineMapping);
    }
    // push another empty line for inline sourcemap
    newFileMappings.push([]);
    return newFileMappings
      .map((line) => line.map((entry) => vlq.encode(entry)).join(','))
      .join(';');
  }
    ...
    for (let operation of patch) {
      ...
      if (prefix.length > 0) {
        ...
        if (!backingClassPath.endsWith('.coffee')) {
          ...
          let customSourceMap = new SourceMap({
            file,
            sources: [backingClassPath],
            sourcesContent: [originalJsContents],
            names: [],
            mappings: this.generateSourceMappings(originalJsContents),
          });

          jsContents += `\n//# sourceMappingURL=${customSourceMap.toUrl()}`;
          ...
          // compare URLS
          let oldURL = jsContentsMap.toUrl();
          let newURL = customSourceMap.toUrl();
          console.log(oldURL);
          console.log(newURL);

          // compare strings
          let oldString = jsContentsMap.toString();
          let newString = customSourceMap.toString();
