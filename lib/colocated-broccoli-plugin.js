  ...
  build() {
    ...
    for (let operation of patch) {
      ...
      if (prefix.length > 0) {
        ...
        // Apply sourcemap only if original content is available
        if (originalJsContents) {

          // It's not clear to me how to format this correctly for coffeescript.
          if (!backingClassPath.endsWith('.coffee')) {
            let file = filePathParts.name + ext;

            // build mappings between source and genereated (with added prefix)

            // start with two empty lines, since the first two lines map to nothing
            let newFileMappings = [[], []];

            let contentLines = originalJsContents.split(/\r?\n/);

            for (let lineIndex = 0; lineIndex < contentLines.length; lineIndex++) {
              let line = contentLines[lineIndex];
              let lineMapping = [];
              for (let columnIndex = 0; columnIndex <= line.length; columnIndex++) {
                lineMapping.push([columnIndex, 0, lineIndex, columnIndex]);
              }
